...
bool isRecordingButtonPressed = false;
...
- (void)changeRecordingButtonBoolean:(bool)isPressed{
    isRecordingButtonPressed = isPressed;
}
...
- (NSArray<NSValue *> *)doWorkOnSampleBuffer:(CMSampleBufferRef)sampleBuffer inRects:(NSArray<NSValue *> *)rects  {
    if(isRecordingButtonPressed == true){
        ...
        for (unsigned long j = 0; j < convertedRectangles.size(); ++j)
        {
            ...
            for (unsigned long k = 48; k < shape.num_parts(); k++) {
                dlib::point p = shape.part(k);
                [ArrayOfPoints addObject:[NSValue valueWithCGPoint:CGPointMake(int(p.x()),int(p.y()))]];
